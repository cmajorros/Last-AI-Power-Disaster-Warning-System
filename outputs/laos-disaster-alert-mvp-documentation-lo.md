...
ສ່ວນ AI intake ສາມາດປ່ຽນຂໍ້ມູນທີ່ເຂົ້າມາໃຫ້ເປັນຮ່າງແຈ້ງເຕືອນທີ່ມະນຸດກວດທານໄດ້. ຮອງຮັບຂໍ້ຄວາມຈາກ Gmail, email ອື່ນໆ, ຂໍ້ຄວາມ WhatsApp Business, ຮູບພາບ, ພາບໜ້າຈໍ, PDF ແລະ ເອກະສານຂໍ້ຄວາມ. ຖ້າຕັ້ງ `OPENAI_API_KEY` ແລ້ວ ລະບົບຈະໃຊ້ OpenAI ເພື່ອອ່ານຫຼັກຖານ ແລະ ສັງເຄາະຮ່າງແຈ້ງເຕືອນ, ກຸ່ມເປົ້າໝາຍ, ກຸ່ມ routing, ຊ່ອງທາງແນະນໍາ, ຂໍ້ຄວາມພາສາອັງກິດ/ລາວ, ຄວາມໝັ້ນໃຈ ແລະ quality flags. ຖ້າບໍ່ມີ key, MVP ຈະໃຊ້ການວິເຄາະ demo ແລະ ສະແດງ flag ໃຫ້ reviewer ເຫັນ.
...
1. ເປີດ `Alerts`.
2. ເລືອກແຫຼ່ງຂໍ້ມູນ: `Gmail`, `Other email`, `WhatsApp Business`, ຫຼື `Upload/manual`.
3. ວາງຂໍ້ຄວາມ email, WhatsApp ຫຼື report ຈາກພື້ນທີ່.
4. Upload ຮູບພາບ, screenshot, PDF ຫຼື text document ຖ້າມີ.
5. ເພີ່ມ reviewer routing notes ເຊັ່ນ `Prioritize elders, children, schools, clinics, rescue boats, and neighboring district equipment`.
6. ກົດ `Analyze intake`.
7. ກວດທານ AI proposal, quality flags, source summary, evidence summary, target audience ແລະ suggested channels.
8. ກົດ `Apply to form` ເພື່ອແກ້ໄຂ fields ເອງ ຫຼື ກົດ `Create AI draft` ເພື່ອສ້າງ alert ສະຖານະ `AI Generated`.
9. ດໍາເນີນຕໍ່ດ້ວຍ `Send for review`, `Approve` ແລະ `Publish`.

Reviewer ສາມາດປ່ຽນ severity, location, target channels, audience routing notes ແລະ messages ກ່ອນອະນຸມັດ. AI intake ບໍ່ publish ຫຼື ສົ່ງຂໍ້ຄວາມອັດຕະໂນມັດ.
...
- AI intake ທີ່ພ້ອມໃຊ້ OpenAI ສໍາລັບ Gmail-style reports, WhatsApp Business webhook content, uploaded images/screenshots, PDFs ແລະ text documents
...
- AI: OpenAI Responses API intake ເມື່ອຕັ້ງ `OPENAI_API_KEY`, ພ້ອມ deterministic local fallback ສໍາລັບ demo
...
- AI intake ໃຊ້ OpenAI ເມື່ອຕັ້ງ `OPENAI_API_KEY`; ຖ້າບໍ່ມີ key ຈະ fallback ເປັນ deterministic demo extraction.
- Gmail ໃນ MVP ແມ່ນ UI source type ແລະ test endpoint; production Gmail ຕ້ອງມີ Google OAuth, Gmail API scopes ແລະ polling ຫຼື Pub/Sub push.
- WhatsApp incoming text ໃນ MVP ແມ່ນ webhook-shaped endpoint; production attachment OCR ຕ້ອງ download media ຜ່ານ Meta Graph API.
...
### Test 3A: ສ້າງ Alert ຈາກ AI Intake

1. ເຂົ້າລະບົບເປັນ `DMH Vientiane Duty Officer`.
2. ເປີດ `Alerts`.
3. ໃນ AI intake panel ເລືອກ `Gmail`.
4. ວາງຂໍ້ຄວາມນີ້:
...
5. ເພີ່ມ routing notes: `Prioritize elders, children, schools, clinics, rescue boats, and neighboring district equipment`.
6. ກົດ `Analyze intake`.
7. ກວດວ່າ proposal ມີ Khammouane/Thakhek, priority groups, SMS/WhatsApp channels ແລະ quality flags ຖ້າບໍ່ມີ OpenAI key.
8. ກົດ `Apply to form`.
9. ແກ້ໄຂ fields ຕາມການພິຈາລະນາຂອງ reviewer.
10. ກົດ `Create AI draft`.
11. ດໍາເນີນຕໍ່ດ້ວຍ `Send for review`, `Approve` ແລະ `Publish`.

Pass condition:

- ສ້າງ alert ສະຖານະ `AI Generated` ຈາກ incoming report ໄດ້.
- Reviewer ປ່ຽນ channels ແລະ routing ກ່ອນອະນຸມັດໄດ້.
- Publishing ຍັງຕ້ອງຜ່ານ approval ແລະ ສ້າງພຽງ notification logs ແບບຈໍາລອງໃນ MVP.

### Test 4: ການບັນທຶກ SMS ແລະ WhatsApp ດ້ວຍເບີທົດສອບ

MVP ປັດຈຸບັນມີ volunteer ທົດສອບທີ່ເບີລົງທ້າຍ `2825`. ໃຫ້ຕັ້ງເບີເຕັມໃນ local secrets ຫຼື Vercel secrets ເທົ່ານັ້ນ.
...
| `OPENAI_API_KEY` | OpenAI API key ສໍາລັບ AI intake ແລະ alert synthesis |
| `OPENAI_INTAKE_MODEL` | OpenAI model ທີ່ໃຊ້ວິເຄາະ intake |
...
| `WHATSAPP_WEBHOOK_VERIFY_TOKEN` | Token ສໍາລັບ WhatsApp webhook verification |
| `WHATSAPP_PHONE_NUMBER_ID` | WhatsApp Business phone number ID |
| `GMAIL_CLIENT_ID` | Gmail OAuth client ID ສໍາລັບ production integration ໃນອະນາຄົດ |
| `GMAIL_CLIENT_SECRET` | Gmail OAuth client secret ສໍາລັບ production integration ໃນອະນາຄົດ |
